...
public class AddressResource {
    ...
    @RequestMapping(value = "/addresss",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public List<Address> getAllAddresss(@RequestParam(required = false) String filter) {
        if ("patients-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where patients is null");
            return StreamSupport
                .stream(addressRepository.findAll().spliterator(), false)
                .filter(address -> address.getPatients() == null)
